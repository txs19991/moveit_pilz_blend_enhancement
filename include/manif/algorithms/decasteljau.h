...
namespace manif
{
  ...
  decasteljau(const std::vector<LieGroup> &trajectory,
              const double u)
  {
    MANIF_CHECK(trajectory.size() > 1,
                "Input trajectory must have more than 1 point!");
    MANIF_CHECK(u >= 0.0 && u <= 1.0,
                "u must be in [0,1]!");

    std::vector<LieGroup> temp;
    unsigned int m = trajectory.size() - 1;
    for (size_t i = 0; i <= m; ++i)
    {
      temp.emplace_back(trajectory[i]);
    }
    ...
    for (size_t k = 1; k <= m; ++k)
    {
      ...
      {
        temp[i] = temp[i].rplus((temp[i + 1].rminus(temp[i])) * u);
      }
    }
