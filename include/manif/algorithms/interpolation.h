...
namespace manif
{
  ...
  {
    return (n >= k) ? (k >= 0) ? (k * 2 > n) ? binomial_coefficient(n, n - k) : k ? binomial_coefficient(n, k - 1) * (n - k + 1) / k
                                                                                  : 1
                               // assert n ≥ k ≥ 0
                               : (throw std::logic_error("k >= 0 !"))
                    : (throw std::logic_error("n >= k !"));
  }
  ...
  T smoothing_phi(const T t, const std::size_t degree)
  {
    ...
    return degree == 1 ? (T(3.) * t2 - T(2.) * t3) : degree == 2 ? (T(10.) * t3 - T(15.) * t4 + T(6.) * t5)
